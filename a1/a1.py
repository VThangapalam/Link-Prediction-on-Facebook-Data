...
def bfs(graph, root, max_depth):
    ...
    node2distances = defaultdict(str)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    ###TODO
    edgevalues = defaultdict(int)
    nodevalues = defaultdict(int)
    children = defaultdict(list)
    nodeLevelSorted =sorted(node2distances, key=node2distances.get, reverse=True)
   
            
    currentlevel =  node2distances[nodeLevelSorted[0]]
    for i in nodeLevelSorted:
        nodevalues[i]=nodevalues[i]+1
        numParents = len(node2parents[i])
        for p in node2parents[i]:
            tup=(i,p)
            
            edgevalues[tuple(sorted(tup))]=nodevalues[i]/numParents
            nodevalues[p]=nodevalues[p]+nodevalues[i]/numParents
        
    nodevalues.pop(root, None)
    return edgevalues
...
def approximate_betweenness(graph, max_depth):
    ...
    """
    ###TODO
    egdeBetweeness = defaultdict(int)
    approxbetweeness=defaultdict(int)
    for node in graph.nodes():
        ...
        for key,value in result.items():
            egdeBetweeness[key]=egdeBetweeness[key]+ value
    
    for key,value in egdeBetweeness.items():
        temp=value/2
        approxbetweeness[key]=temp
...
def is_approximation_always_right():
    ...
    """
    ###TODO
    """ The approximate betweeness takes max_depth into consideration , but in exact computation the entire graph is being considered
    Therefore betweeness calculated through approximation may not always be equal to the exact computation
    """
    s = 'no'
    ##print ("Is approximation always right : no")
    return s
...
def partition_girvan_newman(graph, max_depth):
    ...
    betweenness= approximate_betweenness(TempGraph, max_depth)
    sortedTup = sorted(betweenness.items())
    bt= sorted(sortedTup, key=lambda x: (-x[1],x[0]))
    parts =0
    while(parts<2):
        TempGraph.remove_edge(bt[0][0][0],bt[0][0][1])
        bt.remove(bt[0])
        graphs = list(nx.connected_component_subgraphs(TempGraph))
        parts = len(graphs)
    return graphs
...
def norm_cut(S, T, graph):
    ...
    ncv = (cutVal/volumeS)+ (cutVal/volumeT)
    return ncv
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        ncvVal = norm_cut(partitions[0], partitions[1], graph)
        score.append((depth,ncvVal))
...
def make_training_graph(graph, test_node, n):
    ...
    ###TODO
    tempGraph = graph.copy()
    neighList = sorted(graph.neighbors(test_node))
    for num in range(0,n):
        tempGraph.remove_edge(test_node,neighList[num])
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        ...
        numerator = len(neighbors & neighbors2)
        denom =  len(neighbors | neighbors2)
        if(denom!=0):
            val = numerator/denom
            tup = (node,n)
            edge = tuple(sorted(tup))
            if(n!=node and ((n,node) not in graph.edges()) and ((node,n) not in graph.edges())):
                scores.append((tup,val))

    scorelist= sorted(scores, key=lambda x: (-x[1],x[0][1]))
    return scorelist[:k]


#######bfs for all nodes
def bfs_all(graph, root):
    visited = set()
    node2distances = defaultdict(str)
    node2num_paths = defaultdict(list)
    parents = defaultdict(str)
...
def path_score(graph, root, k, beta):
    ...
    ###TODO
    scores=[]
    node2distances,node2num_paths = bfs_all(graph,root)
    for node in graph.nodes():
        similarity = (beta**node2distances[node])*(node2num_paths[node])
        if(node!=root and ((root,node) not in graph.edges()) and ((node,root) not in graph.edges())):  
            scores.append(((root,node),similarity))
    scorelist= sorted(scores, key=lambda x: (-x[1],x[0][1]))
    return scorelist[:k]
...
def evaluate(predicted_edges, graph):
    ...
    """
    ###TODO
    numberOfpre =len(predicted_edges)
    cnt = 0
    for ed in predicted_edges:
        if(((ed[0],ed[1]) in graph.edges()) or ((ed[1],ed[0]) in graph.edges())):
            cnt = cnt+1
    result = cnt/numberOfpre
    return result
